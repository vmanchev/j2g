...
export class JiraService {
    ...
    async verifyJiraConnection(config: J2GConfig): Promise<boolean> {
        ...
        try {
            response = await fetch(`${config.url}/rest/api/${JIRA_API_VERSION}/myself`, {
                method: "GET",
                headers: {
    ...
    async getIssueData(code: string, config: J2GConfig): Promise<IssueData> {
        ...
        try {
            response = await fetch(`${config.url}/rest/api/${JIRA_API_VERSION}/issue/${code}?fields=summary,issuetype`, {
                method: "GET",
                headers: {
                    Accept: "application/json",
                    Authorization: `Bearer ${config.token}`
