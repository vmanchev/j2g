...
const projectFoundInConfigQuestion = {
    ...
    message: "In your existing configuration, this project is assigned to {jira_url} Jira board. What you want to do?",
    choices: [{
...
const resetQuestion = {
    ...
    name: "reset",
    default: false,
    message: "This will reset j2g global configuration for all Jira boards and all projects. Are you sure you want to continue?",
};
...
async function startConfig() {
    ...
    if (!exisingConfig) {
        ...
        const newConfig = await inquirer.prompt([newUrlQuestion, tokenQuestion]);
    ...
    if (existingProject) {
        ...
        const { existingConfigAction } = await inquirer.prompt([{
            ...
            message: projectFoundInConfigQuestion.message.replace('{jira_url}', existingProject.url),
        }]);

        if (existingConfigAction === ProjectFoundInConfig.REMOVE) {
            configService.removeProject(projectPath);
            loggerService.info('Current project is not part of any j2g configuration')
        }
        return;
    }
    ...
    const projectNotFoundInConfigurationsAnswers = await inquirer.prompt(projectNotFoundInConfigurationsQuestions);

    if (projectNotFoundInConfigurationsAnswers.existingJiraBoard === JiraBoard.NOT_LISTED) {

        if (configService.isExistingJira(projectNotFoundInConfigurationsAnswers.url)) {
            loggerService.error(`Configuraton for ${projectNotFoundInConfigurationsAnswers.url} already exists!`)
            return;
        }

        if (!(await verifyJiraConnection({ ...projectNotFoundInConfigurationsAnswers }))) {
            return;
        }
        ...
        loggerService.info(`Configuration for ${projectNotFoundInConfigurationsAnswers.url} was created`);
        loggerService.info(`Current project was assigned to ${projectNotFoundInConfigurationsAnswers.url} configuration`);
    } else {
        configService.addProject(projectPath, projectNotFoundInConfigurationsAnswers.existingJiraBoard);
        loggerService.info(`Current project was assigned to ${projectNotFoundInConfigurationsAnswers.existingJiraBoard} configuration`);
    }
}
...
async function setNewToken() {
    ...
    configService.updateTokenForBoard(answers.url, answers.token);
...
async function verifyJiraConnection(config: J2GConfig): Promise<boolean> {
    const jiraService = new JiraService(
        new LoggerService(LogLevel.VERBOSE),
    );
